    ...
    createRecipe: async (
      ...
      try {
        const response = await fetch(
          `https://api.spoonacular.com/recipes/complexSearch?query=${title}&apiKey=${process.env.API_KEY}`
        );
        if (!response.ok) {
        ...
        if (data.results && data.results.length > 0) {
          ...
          recipe.ingredients = apiRecipe.extendedIngredients.map(
            (ing) => ing.originalString
          );
